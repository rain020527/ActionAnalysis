...
if __name__ == '__main__':
    print(f"-------------------------\n"
          ...
          f"| Event 4 (Pose start):     {KEY_Event4}\n"
          f"| Event 5 (Pose ready):     {KEY_Event5}\n"
          f"| Event 6 (Pose end):     {KEY_Event6}\n"
          f"| Zoom:                {KEY_Zoom}\n"
          f"| Save:                {KEY_Save}\n"
    ...
    while True:
        ...
        elif key & 0xFFFF == ord(KEY_Event4):
            df[current_frame_idx]['Event'] = 4
            print(f"{KEY_Event4} Pressed At Frame: {current_frame_idx}")
            cv2.imwrite(str(current_frame_idx) + '_start_' + '.jpg', img)

        elif key & 0xFFFF == ord(KEY_Event5):
            df[current_frame_idx]['Event'] = 5
            print(f"{KEY_Event5} Pressed At Frame: {current_frame_idx}")
            cv2.imwrite(str(current_frame_idx) + '_ready_' + '.jpg', img)

        elif key & 0xFFFF == ord(KEY_Event6):
            df[current_frame_idx]['Event'] = 6
            print(f"{KEY_Event6} Pressed At Frame: {current_frame_idx}")
            cv2.imwrite(VIDEO_NAME + '_end_' + str(current_frame_idx) + '.jpg', img)
