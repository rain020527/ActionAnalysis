...
class Smash_VibePklParser(VibePklParser):
    ...
    def time_slice(self):
        ...
        if video_hit_frame+1 < df_model3D.shape[0]:
            ...
            logging.warning('t_strike speed is random')
        ...
        for i in range(df_model3D.shape[0]-1):
            if df_model3D['Event'][i] == 1:
                mul = (float(df_model3D['Y'][i+1]) - float(df_model3D['Y'][i])) * (float(df_model3D['Y'][i]) - float(df_model3D['Y'][i-1]))
                if mul < mul_min:
                    mul_min = mul
                    video_hit_frame = df_model3D['Frame'][i]
        """
        ...
        rwrist_court_zmin = float('inf')
        t_end = 0
        for frameidx in range(tstrike_fid, tstrike_fid + int(self.fps), 1):
            listidx = self.getListIdxByFrameIdx(frameidx)
...
def main():
    pklparser = Smash_VibePklParser(DIRNAME, sys.argv[1], sys.argv[2], sys.argv[3]) # pkl, camera cfg, src videoname
